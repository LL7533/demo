...
$openId = !empty($request->post['openId']) ? $request->post['openId'] : '';
$iChannelId = !empty($request->post['channelId']) ? $request->post['channelId'] : '';
$phone = !empty($request->post['phone']) ? $request->post['phone'] : '';
if(!empty($openId) && !empty($iChannelId)){
    $sdkPath = dirname(__FILE__).'/../../../service/sdk/sdk.class.php';
    require_once($sdkPath);
    $sdk =  \sdk::Instance();
    $arr = $sdk->call("user/checkBonusNew",['openId'=>$openId,'channelId'=>$iChannelId,'phone'=>$phone]);
...
$http->on('request', function ($request, $response) {
 ...
 //   require_once($sdkPath);
 //   $sdk =  \sdk::Instance();
    if(empty($request->get['openId']) || empty($request->get['channelId'])){
        $arrReturn = ['ret'=>-1,'sub'=>-1,'msg'=>'params error!'];
        $response->end(json_encode($arrReturn));
    }
    $openId = $request->get['openId'];

    $iChannelId = $request->get['channelId'];
